        ...
        <>
            <View className="my-2 w-full">
                ...
                >
                    ...
                    >
                        {selectedItem
                            ? `${items.find((i) => i?.[item_value] === selectedItem)?.[
                            item_label
                            ]}`
                            : placeholder
                                ? placeholder
            ...
            <RBSheet
                ...
                height={
                    items.length < 1
                        ? 200
                        : items.length * 75 + 130 > SIZES.height - pickerOffset
                ...
                <ScrollView contentContainerStyle={{ flexGrow: 1, marginVertical: 20 }}>
                    ...
                    {items.map((item, index) => (
                        <TouchableOpacity
                            ...
                            activeOpacity={0.4}
                            className={`p-3  mx-4 my-1 rounded ${selectedItem && item?.[item_value] === selectedItem
                                ? "border-blue-700 border-2"
                                : "border-gray-300 border"
                                }`}
                        >
                            <Text
                                className={`font-Jost text-lg ${selectedItem && item?.[item_value] === selectedItem
                                    ? "text-gray-700"
                                    : "text-gray-500"
                                    }`}
                            ...
                            {item?.[item_subtitle] && (
                                <Text
                                    className={`text-sm mt-1 font-Jakarta ${selectedItem && item?.[item_value] === selectedItem
                                        ? "text-gray-700"
                                        : "text-gray-500"
                                        }`}
                                    style={{ textAlign: "center" }}
